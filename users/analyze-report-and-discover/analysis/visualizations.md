...
## Arc gauge
An *arc gauge* can display how a value measures between two other values.
...
## Bubble chart
A *bubble chart* vizualies three of four values groupen on a single category. 
...
## Circle packing
A *circle packing* is a layout of circles, where the size of a circle correspond to a value. 
The color may correspond to the grouping by a single category, or to the value. 
It is useful for identifying deviating values.

Actions can be applied to Header, Content, Button, and Context Menu.

## Circular distribution
A *circular distribution* is used to display a single value grouped by one or two categories, in a circle.
...
## Deviation table
A *deviation table* is used to list a percentage deviation between two values grouped by a single category. The table shows the group name, current value and deviation.
...
## Dropdown
A *dropdown* visualizes multiple categories. It supports n categories, and shows a dropdown for each, containing the objects from the category data source.
...
A *full stacked column chart* has the same functionality as a [stacked column chart](#stacked-column-chart).
The data points are visualized in vertical columns rather than in horizontal bars.

## Heatmap
A *heatmap* visualizes a single value grouped by two category
...
## Speedometer gauge
A *speedometer gauge* displays a value in a speedometer gauge. The first value is the value to be displayed. The second and third values are the start and end of the gauge, respectively. 
...
### Formatting
The Value Grid can be formatted in a various of ways, on row level (applied to all cells in that row), column level (applied to all cells in that column) and on cell/value level (applied to a single cell). Some formatting properties (not exhaustive) are:
* Row height
* Column height
* Border format
* Background color
