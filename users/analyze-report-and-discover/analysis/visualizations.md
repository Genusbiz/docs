...
The table shows the group name, current value and deviation.
...
Adjusting font size and color is supported, as is defining a suffix and number format of the values.
...
It supports n categories, and shows a dropdown for each, containing the objects from the category data source.
...
The bars will all be the same height, so they will not be used to compare the total value of objects in the primary category, but to compare the distribution of *Auto Series* objects or values in them.
...
The grid allows data to be grouped by an unlimited number of columns. 
When a grid is grouped by a single column, rows that have identical values in this column are arranged into groups. 
Each group is identified by a group row. 
